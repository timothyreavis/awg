...
function isKnownVaultAgentsTemplate(body: string): boolean {
  ...
  return [
    vaultAgentsTemplate(),
    agentsTemplate(),
    priorVaultAgentsTemplateWithoutQueueCoord(),
    priorVaultAgentsTemplateWithQueueOnlyWithoutCloseoutHygiene(),
...
function priorVaultAgentsTemplateWithoutQueueCoord(): string {
  return vaultAgentsTemplate().replace("- Run `awg queue next --json` and `awg coord status --json` when selecting undirected next work; use `awg queue show <item-id> --json` and `awg coord check --target <id> --json` before acting on claimed or multi-node work.\n", "");
}

function priorVaultAgentsTemplateWithQueueOnlyWithoutCloseoutHygiene(): string {
  return `# AWG Agent Instructions
