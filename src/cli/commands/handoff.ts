...
export async function handoffCommand(parsed: ParsedArgs): Promise<void> {
  ...
  if (parsed.flags.compact) {
    ...
    for (const sectionName of ["activeRun", "mostRecentRun", "workQueues", "handoffFollowup", "currentFocus", "activeTasks", "blockersAndRisks", "recommendedNextActions"]) {
      ...
      console.log(`\n${section.section}:`);
      for (const item of first) console.log(`- ${formatHandoffItem(item as Record<string, unknown>, item)}`);
    }
    return;
  }
