...
Use the project-local .awg vault only. Start with \`awg handoff\`, run \`awg release current\` after install or upgrade, check \`awg vault topology --json\` before cross-project work, then \`awg run start --goal "<goal>"\`. Use \`awg search\` before creating nodes, inspect \`awg template status --goal "<goal>" --json\`, scope work with \`awg lens task --goal "<goal>"\`, and inspect full surfaced nodes with \`awg node show <node-id> --json\` when needed. Use configurable lenses only for repeated context shapes: list/show them first, run reviewed lenses read-only, and update near-duplicates instead of adding more. Capture the consequence, not the conversation; preserve durable decisions, requirements, accepted plans, constraints, risks, blockers, tasks, evidence, source-of-truth boundaries, and actionable feedback, while leaving brainstorming in chat or capturing it only as a \`needs_review\` note/question with a \`hypothesis\` tag when useful. Use \`awg quick note|task|risk|question|decision "summary"\` for small durable captures. Update existing nodes, use structured fields/safe node-local blocks/freshness/anchors when useful, use authored views for recurring multi-node human review surfaces and configurable lenses for recurring agent retrieval recipes, add run notes, attach evidence for completed work, write only to the current explicit target vault, then run \`awg build\`, \`awg doctor --fix-suggestions --json\`, and \`awg run finish --status completed|partial|blocked|failed --summary "..." --auto-handoff\`. If forced, document why.
`;
}
