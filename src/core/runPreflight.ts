...
export function preflightRun(graph: CompiledGraph, run: AgentRun): RunPreflightResult {
  ...
  addNodeWarning("AWG_RUN_CLOSEOUT_CANDIDATE_TOUCHED", "Touched node is a closeout candidate.", touchedAttention.filter((item) => item.closeoutReasons.length > 0).map((item) => item.nodeId), "Run `awg closeout run --json` and close or acknowledge touched lifecycle debt.");
