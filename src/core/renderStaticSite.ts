...
export function queryNodes(nodes: AwgNode[], diagnostics: Diagnostic[], query: ViewerQuery = {}, attentionIndex?: CompiledGraph["attention_index"]): AwgNode[] {
  ...
  let result = nodes.filter((node) => {
    ...
    if (query.needsAttention) {
      ...
      const hasAttentionIndex = Boolean(attentionIndex?.items.length);
