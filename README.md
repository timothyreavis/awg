...
- `#/queues` for derived agent work queues
...
awg add claim --title <title> --claim <statement> [--kind <kind>] [--source-of-truth <ref>] [--review-after <date>] [--evidence-required] [--tag <tag>] [--run <run-id>|--no-run] [--json]
...
awg add evidence --target <node-id> --summary <summary> [--source <source>] [--command <command>] [--path <path>] [--url <url>] [--status <passed|failed|unknown|superseded>] [--rel <supports|contradicts|verified_by|derived_from>] [--expires-at <date>] [--review-after <date>] [--reliability <low|medium|high>] [--excerpt <text>] [--redacted] [--run <run-id>|--no-run] [--json]
awg verify <node-id> --summary <summary> [--source <source>] [--command <command>] [--path <path>] [--url <url>] [--status <passed|failed|unknown>] [--rel <supports|contradicts>] [--expires-at <date>] [--review-after <date>] [--reliability <low|medium|high>] [--excerpt <text>] [--redacted] [--run <run-id>|--no-run] [--json]
awg claim status <node-id> [--json]
awg claims [--status <status>] [--kind <kind>] [--tag <tag>] [--limit <n>] [--json]
...
awg queue next [--goal <goal>] [--queue <id>] [--limit <n>] [--autonomous] [--include-human-review] [--include-claimed] [--mine] [--json]
...
awg closeout mark <node-id> --status <completed|resolved|archived|superseded|needs_review> --reason <text> [--target <node-id>] [--rel <rel>] [--expect-updated-at <iso>] [--force] [--run <run-id>|--no-run] [--json]
...
Claims are normal AWG nodes, usually `type: "claim"` with structured `fields.claim`, `fields.claim_kind`, and `fields.verification_status`. Use `awg add claim` for statements that may guide future work, and use `awg verify <node-id>` or `awg add evidence --target <node-id>` to attach proof. The compiler derives `graph.claim_index`, `graph.evidence_index`, `.awg/compiled/indexes/claims.json`, and `.awg/compiled/indexes/evidence.json` without replacing append-only canonical logs. Supported proof relation directions are `supports` and `contradicts` from evidence/source to target, and `verified_by` and `derived_from` from target to evidence/source. `awg claim status` and `awg claims` expose the derived trust records.

Work queues are read-only projections over normal AWG graph state. The compiler derives `graph.work_queue_index` and `.awg/compiled/indexes/work-queues.json` from maintenance inbox items, nodes, runs, claims, evidence, diagnostics, topology data, and the derived attention index. Built-in queues are `next`, `autonomous`, `human_review`, `blocked`, `evidence_needed`, `maintenance`, `stale_review`, `risk_review`, and `handoff_followup`. Queue items have stable `wq:<queue>:<hash>` ids, priority, severity, source attribution, reasons, suggested commands, autonomous safety, human review flags, blocker metadata, timestamps, and compact attention metadata when relevant.

Use `awg queue next --json` and `awg coord status --json` after `handoff`, `template status`, and `lens task` when selecting undirected next work. `awg queue next` excludes human-review items and other-run active exclusive coordination claims by default; add `--include-human-review` or `--include-claimed` to see them, and use `--mine` for work claimed by the current run. `awg queue show <item-id> --json` expands related coordination claims, collisions, and handoffs alongside nodes, runs, inbox items, diagnostics, claims, and evidence.

Coordination is advisory, append-only work intent. `awg coord claim` records an `exclusive`, `shared`, or `watch` claim on a node, queue item, run, or vault reference. `coord check` reports active exclusive claims and collisions before acting, `coord release` closes claims as completed/released/abandoned/blocked, and `coord handoff` leaves a durable pass-off note. It is not a hard lock, permission model, scheduler, sync service, or substitute for evidence.
...
`awg inbox` derives a deterministic maintenance queue from compiled graph state. It is read-only and local. Item kinds include `stale`, `needs_review`, `duplicates`, `orphans`, `claims`, `evidence`, `questions`, `risks`, `blockers`, `decisions`, `topology`, and `hygiene`. `awg inbox --json` returns stable item objects with ids, severity, priority, involved nodes/runs/vaults, reasons, conservative suggested commands, and autonomous/human-review flags. `--kind` filters the queue and `--limit` caps output. `awg doctor --fix-suggestions --json`, `handoff`, task lenses, resume lenses, run preflight, and the viewer reuse this inbox where safe.
...
`awg run finish --status completed` runs a deterministic preflight. It warns about completed tasks without evidence, evidence-required nodes without evidence, unresolved claim contradictions, missing claim evidence, stale/expired claims or evidence, active risks/blockers, proposed decisions, orphan or duplicate-ish nodes created during the run, high-priority inbox items affecting touched nodes, missing notes, missing changes, missing handoff, stale touched nodes, and doctor warnings affecting touched nodes. Completed runs with substantive unresolved preflight warnings require `--force`; partial, blocked, failed, and abandoned runs remain usable and report warnings. `--auto-handoff` records and prints a compact handoff after finishing, and JSON output includes finish, preflight, and handoff data. If `--force` is used, document why in the summary or a run note.
...
Use `awg add evidence --target <node-id> --summary "..."` when claiming work is complete or verified. It validates the target, creates a generic `evidence` node, links it to the target with a proof edge, and records inline evidence on the target so current diagnostics can recognize completed task evidence. `--rel supports|contradicts` writes evidence-to-target edges; `--rel verified_by|derived_from` writes target-to-evidence edges.
...
7. Derives attention before work queues, so queues consume attention without feeding back into base attention.
8. Generates graph JSON, diagnostics, resume lens, current view, compiled indexes, and static viewer files.
