...
awg queue next [--goal <goal>] [--queue <id>] [--limit <n>] [--autonomous] [--include-human-review] [--json]
...
Use `awg queue next --json` after `handoff`, `template status`, and `lens task` when selecting undirected next work. `awg queue next` excludes human-review items by default; add `--include-human-review` to see them. `--goal` uses deterministic text matching only. `awg queue show <item-id> --json` expands the related nodes, runs, inbox items, diagnostics, claims, and evidence. Queue commands are read-only: they do not claim, reserve, lock, assign, or execute work.
