...
V2.2 will strengthen claim and verification semantics without adding a parallel storage model. Claim-bearing knowledge should remain normal AWG nodes, usually `type: "claim"` or existing node types with structured claim fields. Evidence should remain normal `type: "evidence"` nodes linked with existing relations such as `supports`, `verified_by`, `contradicts`, and `derived_from`. See `docs/spec/awg-claims-evidence.md` for the planned claim, evidence, verification, diagnostics, and agent guidance contract.
