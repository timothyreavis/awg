...
V2.4.1 is planned to make adaptive onboarding discoverable through existing lenses. `lens resume` and `lens task` should surface missing or unreviewed operating-template guidance when relevant, and configurable lenses may use existing template, queue, coordination, diagnostics, and static sources to create vault-local onboarding review lenses. AWG should not ship fixed role/domain lenses as the main adaptation strategy.
